...
const RoomDetail = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // 2. Kiểm tra Tim (Cần đăng nhập)
                const token = localStorage.getItem('accessToken');
                // Thêm kiểm tra token hợp lệ để tránh gửi request "rác" lên server
                if (user && token && token !== "null" && token !== "undefined") {
                    try {
                        const favStatus = await favoriteService.checkStatus(id);
                        // Lưu ý: Vì Backend checkStatus trả về ResponseEntity<Boolean>
                        // nên favStatus.data sẽ là true/false trực tiếp.
                        setIsFavorite(favStatus.data);
                    } catch (favErr) {
                        // Nếu lỗi 401/400 ở đây, ta chỉ cần log và coi như chưa favorite
                        console.warn("Không thể kiểm tra trạng thái yêu thích:", favErr);
                        setIsFavorite(false);
                    }
                } else {
                    setIsFavorite(false);
    ...
    const handleToggleFavorite = async () => {
        ...
        try {
            ...
            // Backend trả về Map.of("isFavorited", isFavorited)
            const currentStatus = res.data.isFavorited;
    ...
    const fetchVideoRooms = async () => {
        ...
        try {
            ...
            const allVideoRooms = res.data?.content || [];
            ...
            setVideoRooms(filteredAndSorted.slice(0, 4));
        } catch (error) {
                    ...
                    <Col xs={24} lg={16}>
                        ...
                        <div className="bg-black rounded-xl overflow-hidden relative mb-2 h-[450px] flex items-center justify-center group shadow-lg border border-gray-200">
                            {activeMedia.type === 'video' ? (
                                ...
                                <Image
                                    ...
                                    preview={{
                                        mask: <div className="flex items-center gap-2 text-white font-bold"><PlusOutlined /> XEM TO</div>
                                    }}
                                />
                            )}
                        ...
                        <div className="flex gap-3 overflow-x-auto mb-6 pb-2 scrollbar-hide">
                            ...
                            {room.videoUrl && (
                                ...
                                >
                                    <img src={room.images?.[0]} className="w-full h-full object-cover blur-[1px]" alt="v-thumb" />
                        ...
                        <Card className="shadow-sm border-none mb-4 rounded-lg">
                            ...
                            <div className="flex justify-between items-center mb-4">
                                ...
                                {/* Cụm nút So sánh và Lưu tin nằm cạnh nhau */}
                        ...
                        <div ref={comparisonRef} className="mt-8">
                            <Card
                                className="shadow-sm border-none rounded-lg bg-white mb-8 border-l-4 border-l-orange-400"
                            ...
                            >
                                <div className="flex items-center justify-between mb-6">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <h3 className="text-lg font-bold m-0">Phòng giá tốt nhất khu vực</h3>
                                            <p className="text-gray-500 text-xs m-0">Bán kính 3km quanh đây</p>
                                        </div>
                                    </div>
                                    {cheaperRooms.length > 0 && <Tag color="orange" className="font-bold">GỢI Ý TỐT</Tag>}
                                </div>

                                {hasSearched && cheaperRooms.length === 0 ? (
                                    // HIỂN THỊ KHI KHÔNG CÓ PHÒNG RẺ HƠN
                            ...
                            <Card className="shadow-sm border-none rounded-2xl overflow-hidden">
                                <div className="flex items-center gap-4 mb-5">
                                    <Avatar
                                        ...
                                        icon={<UserOutlined />}
                                        className="shrink-0 border-2 border-orange-50 shadow-sm"
                                    />
                                    <div className="min-w-0">
                                        <div className="font-bold text-gray-900 text-[16px] flex items-center gap-1.5">
                                            <span className="truncate">{room.landlordName || "Chủ trọ"}</span>
                                ...
                                >
                                    {showPhone ? room.landlordPhone : "Bấm để hiện số"}
                                </Button>
                            </Card>
