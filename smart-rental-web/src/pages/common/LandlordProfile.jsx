...
const LandlordProfile = () => {
  ...
  const handleStartChat = () => {
    ...
    // 3. Điều hướng sang Messenger và truyền ID đối phương để mở chat ngay
    navigate('/messenger', { state: { partnerId: profile.id } });
  };
            ...
            {getSortedRooms().map(room => {
                ...
                return (
                  <Col xs={24} sm={12} md={8} lg={6} key={room.id}>
                    <Card 
                      hoverable={!rented} 
                      className={`overflow-hidden border border-gray-200 shadow-sm transition-all rounded-lg h-full flex flex-col group ${rented ? 'bg-gray-100 opacity-90' : 'hover:shadow-lg bg-white'}`}
                      ...
                      cover={
                        <div className="relative h-44 w-full overflow-hidden">
                          ...
                          <div className="absolute bottom-2 right-2 bg-black/60 text-white text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1 backdrop-blur-sm">
                            <CameraFilled /> {room.images?.length || 0}
                          </div>

                          {room.isPro && !rented && (
                              <div className="absolute top-2 left-2 bg-yellow-400 text-yellow-900 text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">
                                  PRO
                              </div>
                          )}
                        </div>
                    ...
                    >
                      <div className="flex-grow">
                        <h3 className={`text-[15px] font-bold line-clamp-2 mb-2 min-h-[44px] leading-snug ${rented ? 'text-gray-500' : 'text-gray-800'}`}>
                            {room.title}
                        </h3>
